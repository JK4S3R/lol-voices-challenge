...
// DASHBOARD
// ============================================================
let dashCurrentMode = 'normal'; // filtre mode dans le dashboard

function buildAreaChart(scores) {
    // Génère un SVG "aire sous la courbe" à partir d'un tableau de scores
    ...
    return `
        <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block" preserveAspectRatio="none">
            ...
            ${dots}
            <text x="${PAD}" y="${H-2}" fill="#555" font-size="7">${n} partie${n>1?'s':''}</text>
            <text x="${W-PAD}" y="${H-2}" fill="#555" font-size="7" text-anchor="end">max: ${max}</text>
        </svg>
    `;
}
...
async function showDashboard() {
    ...
    // Stats champions (globales, pas filtrées par mode)
    ...
    // Leaderboard global pour calculer le rang de l'utilisateur
    ...
    if (!games || games.length === 0) {
        content.innerHTML = modeTabsHTML + `<p style="color:#888;text-align:center;padding:20px">Aucune partie en mode ${dashCurrentMode === 'survival' ? 'Survie' : 'Normal'}.</p>`;
        attachDashTabs();
        return;
    }
    ...
    const rankHTML = myRank > 0
        ? `<div class="dash-rank">🏆 Tu es <strong>${myRank}${myRank === 1 ? 'er' : 'ème'}</strong> sur ${totalPlayers} en ${dashCurrentMode === 'survival' ? 'Survie' : 'Normal'} · ${lang.toUpperCase()} · ${difficulty === 'easy' ? 'Facile' : 'Difficile'}</div>`
        : '';

    // Graphique : 20 dernières parties (en ordre chronologique)
...
function attachDashTabs() {
    document.querySelectorAll('.dash-tab').forEach(btn => {
        btn.onclick = () => {
            dashCurrentMode = btn.dataset.mode;
            showDashboard();
        };
    });
}
